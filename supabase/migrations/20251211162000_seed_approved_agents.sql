-- Temporarily disable RLS on agents table for seeding
ALTER TABLE agents DISABLE ROW LEVEL SECURITY;

-- Insert sample approved agents
...
CREATE POLICY "Agents insert via service role" ON agents
  FOR INSERT
  WITH CHECK (true);

CREATE POLICY "Agents update via service role" ON agents
  FOR UPDATE
  USING (true)
  WITH CHECK (true);
